...
FROM 
    public.countries c
JOIN 
    public.locations l ON c.country_id = l.country_id
JOIN 
    public.departments d ON l.location_id = d.location_id
JOIN 
    public.employees e ON d.department_id = e.department_id
GROUP BY 
    c.country_name
ORDER BY 
    c.country_name;
...
SELECT 
    d.department_name,
    COALESCE(COUNT(dp.dependent_id), 0) AS num_dependents
FROM 
    public.departments d
LEFT JOIN 
    public.employees e ON d.department_id = e.department_id
LEFT JOIN 
    public.dependents dp ON e.employee_id = dp.employee_id
GROUP BY 
    d.department_name
ORDER BY 
    num_dependents DESC,
