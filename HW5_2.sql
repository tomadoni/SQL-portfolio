...
-- Add UNIQUE constraint for department_name
ALTER TABLE public.departments
ADD CONSTRAINT unique_department_name UNIQUE(department_name);

-- Create the function to get the average salary of employees in a department
CREATE OR REPLACE FUNCTION public.f_GetAvgDeptSalary(_department_name VARCHAR(200))
RETURNS DECIMAL(10,2)
AS $BODY$
DECLARE
    avg_salary DECIMAL(10,2);
BEGIN
    -- Check if department_name is NULL
    IF _department_name IS NULL THEN
        RETURN -1.00;
    END IF;

    -- Get the average salary of employees in the specified department
    SELECT COALESCE(AVG(e.salary), 0.00) INTO avg_salary
