...
-- Part A: INSERT/UPDATE BEFORE-trigger for EACH row on public.departments table
CREATE OR REPLACE FUNCTION check_department_head()
RETURNS TRIGGER AS
$$
DECLARE
    dept_id INTEGER;
BEGIN
    IF NEW.department_head IS NOT NULL THEN
        -- Check if the department head belongs to its own department
        SELECT department_id INTO STRICT dept_id
        FROM public.employees
        WHERE employee_id = NEW.department_head;
        
        IF dept_id IS NULL THEN
            -- Assign the department to the department head
            UPDATE public.employees
            SET department_id = NEW.department_id
            WHERE employee_id = NEW.department_head;
        ELSIF dept_id <> NEW.department_id THEN
            -- Department head belongs to another department, rollback the transaction
            RAISE INFO 'Department Head belongs to another department.';
            RETURN NULL;
        END IF;
    END IF;
...
-- Create the trigger
CREATE TRIGGER department_head_trigger
BEFORE INSERT OR UPDATE ON public.departments
FOR EACH ROW
EXECUTE FUNCTION check_department_head();

-- Part B: UPDATE BEFORE-trigger for EACH row on public.employees table
CREATE OR REPLACE FUNCTION check_department_head_update()
RETURNS TRIGGER AS
$$
BEGIN
    IF TG_OP = 'UPDATE' THEN
        -- Check if the employee is a department head and changing department_id
        IF OLD.department_id = NEW.department_id AND OLD.department_id IS NOT NULL THEN
            -- Department head is changing department_id, rollback the transaction
            RAISE INFO 'Changing department for a Department Head is not allowed.';
            RETURN NULL;
        END IF;
        
        -- Check if the employee is a department head and changing department_id to NULL
        IF OLD.department_id IS NOT NULL AND NEW.department_id IS NULL THEN
            -- Setting department to NULL for a Department Head is not allowed, rollback the transaction
            RAISE INFO 'Setting department to NULL for a Department Head is not allowed.';
            RETURN NULL;
        END IF;
    END IF;
...
-- Create the trigger
CREATE TRIGGER department_head_update_trigger
BEFORE UPDATE ON public.employees
FOR EACH ROW
EXECUTE FUNCTION check_department_head_update();


-- Test Case For Part A
